 ...
 */
public fun Position.toOffset(content: CharSequence): Int {
    val lineRanges = content.lineRanges(true)
    var offset = 0
 ...
 */
public fun CharSequence.getCurrentLineBeginning(offset: Int): Int {
 ...
 */
public fun CharSequence.getCurrentLineEndExclusive(offset: Int, includeLineSeparators: Boolean): Int {
 ...
 */
public fun CharSequence.lineRanges(includeLineSeparators: Boolean, after: Int = 0): Sequence<TextRange> = sequence {
 ...
 */
public fun Char.isLineBreak(): Boolean {
    return this == '\n' || this == '\r'
}
 ...
 */
private fun CharSequence.getCurrentLineEndExclusiveWithoutLineBreaks(offset: Int): Int {
    var current = offset
    while (current < length) {
        if (this[current].isLineBreak()) {
            return current
 ...
 *         `false` otherwise.
 */
public fun TextRange.isNotBlank(text: String): Boolean {
 ...
 */
public suspend fun <Path, Document> DocumentProvider<Path, Document>.charsByPath(path: Path): CharSequence? {
 ...
 */
public suspend fun <Path, Document> DocumentProvider<Path, Document>.textByPath(path: Path): String? {
